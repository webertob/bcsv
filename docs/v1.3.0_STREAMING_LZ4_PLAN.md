...
Writer:
  ...
  ...
  Last Packet → calculate payload checksum → store in FileIndex.lastPacketPayloadChecksum
...
Reader:
  ...
  ...
  Read Last Packet → validate with FileIndex.lastPacketPayloadChecksum ✓
```
...
class Writer {
    ...
    void close() {
        ...
        // Write file index
        writeFileIndex(packetIndex_, lastPacketChecksum_, totalRows_);
    }
    
    void writeFileIndex(const std::vector<PacketIndexEntry>& entries,
...
- [ ] Define `FileIndex` structure in `file_index.h`
...
- [ ] Implement `writeFileIndex()` method
- [ ] Write "BIDX" magic
...
- [ ] Implement `readFileIndex()` method
...
- [ ] At packet terminator:
  ...
  - Or validate last packet against `FileIndex.lastPacketPayloadChecksum`
...
3. Define `FileIndex` structure with footer
...
3. **Define file index structure** (`include/bcsv/file_index.h`)?
