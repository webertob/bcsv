...
| **Encoding concerns leak into Reader/Writer** | Writer does ZoH repeat detection (byte comparison) and packet lifecycle orchestration. Change-flag management is codec policy and should stay internal to codec/row internals. |
...
│  RowCodecDelta002<L, P>              (Item 14 — type-grouped,      │
│  │  ├─ prev/grad state                 delta/VLE, per-col state)   │
...
│      RowCodecFlat001<L, P>,                                          │
│      RowCodecZoH001<L, P>                                            │
│      // future: RowCodecDelta002<L, P>, RowCodecFlat002<L, P>, ...  │
│  >;                                                                  │
│                                                                      │
├──────────────────────────────────────────────────────────────────────┤
...
| `RowCodecDelta002<L, P>` | Binary delta (type-grouped, VLE prediction) | Item 14 |
...
- `RowCodecDelta002`: `reset()` clears prev/grad buffers and resets row counter.
...
| `Writer: public change-flag calls` | writer.hpp L246-250 | `RowCodecZoH001::reset()` / `serialize()` internal tracking flow |

### 2.9 What Stays Where

| Component | Keeps |
|-----------|-------|
| **Row classes** | `bits_`, `data_`, `strg_`, `layout_&`, `get()`, `set()`, `visit()`, `visitConst()`, `clear()`, `clone()`, `assign()`, internal codec-facing tracking helpers |
...
| **Item 13/14 (delta)** | `RowCodecDelta002` with type-grouped loops, combined header codes, and gradient state, in its own file. Flat/ZoH codecs don't carry any of this weight. `reset()` clears delta history at packet boundaries. |
...
| 3.2 | Add `codec_: RowCodecVariant` member to Writer. In `open()`, construct the appropriate variant member based on `FileFlags::ZERO_ORDER_HOLD`. In `writeRow()`, replace `row_.serializeTo(buffer)` / `row_.serializeToZoH(buffer)` with `std::visit([&](auto& c) { return c.serialize(row_, buffer); }, codec_)`. Remove row-level public change-flag calls and `row_buffer_prev_` comparison. In `openPacket()`, call `std::visit([](auto& c) { c.reset(); }, codec_)`. | **All existing tests pass** — 343 GTest + 76 C API + Row API. |
...
**TrackingPolicy** configures row/codec behavior (including internal tracking state shape), not a public row change-tracking API contract.
...
|---|---|---|
| **Disabled** | Natural fit. Full decode with no exposed tracking API. | ZoH codec reads wire change-header into **internal** bitset. Decodes only changed columns. Extracts bool values → `row.bits_[boolIdx]`. No per-column change flags are surfaced. |
| **Enabled** | Full decode. Codec marks internal non-BOOL tracking bits as changed (implementation detail). | Natural fit. Wire change-header → `row.bits_` directly (fast alias, same sizing) for internal tracking and bool values. |
...
if constexpr (isTrackingEnabled(Policy)) {
    row.trackingSetAllChanged();  // internal helper, codec-facing only
}
```
...
| **`row_codec_flat001.h/.hpp`** | Minor: Enabled path may need post-deserialize internal tracking hook |
...
| **7c** | Add post-deserialize internal tracking hook for Flat+Enabled path. | Build succeeds |
