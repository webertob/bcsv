...
protected:
    ...
    void SetUp() override {
        ...
        test_dir_        = (fs::path("bcsv_test_files") / (std::string(info->test_suite_name()) + "_" + info->name())).string();
...
TEST_F(BCSVTestSuite, Performance_FlexibleVsStatic) {
    const size_t perf_rows   = 50000; // Larger dataset for reliable performance measurement
    ...
    auto flex_us   = std::chrono::duration_cast<std::chrono::microseconds>(flex_write_time).count();
    auto static_us = std::chrono::duration_cast<std::chrono::microseconds>(static_write_time).count();

    std::cout << "\nPerformance Results for " << perf_rows << " rows:" << std::endl;
    std::cout << "  Flexible interface: " << flex_us << "us" << std::endl;
    std::cout << "  Static interface: " << static_us << "us" << std::endl;
    std::cout << "  File size: " << fs::file_size(flex_file) << " bytes" << std::endl;

    // Both should complete successfully
    EXPECT_GT(flex_us, 0);
    EXPECT_GT(static_us, 0);
}
...
protected:
    ...
    void expectStringEq(const std::string& actual, const std::string& expected, const std::string& message = "") {
        ...
        if (actual.length() > 25)
            actual_preview += "...";
        if (actual.length() > 50)
            actual_preview += actual.substr(actual.length() - 25);

        std::string expected_preview = expected.substr(0, 25);
        if (expected.length() > 25)
            expected_preview += "...";
        if (expected.length() > 50)
            expected_preview += expected.substr(expected.length() - 25);
