...
TEST_F(CodecZoH001DynamicTest, Serialize_AllChanged) {
    ...
    row.changes().set();  // mark all as changed (like first row in packet)
    row.set<bool>(0, true);
...
TEST_F(CodecZoH001DynamicTest, Serialize_BoolTrueToFalse_HeaderOnlyNonEmpty) {
    ...
    // Create a true->false transition on a bool column with no non-bool changes.
    row.set<bool>(0, true);
    row.changes().reset();
    ...
    // Must serialize a header-only row (non-empty), even though wire header bits are all zero.
    EXPECT_FALSE(wire.empty());
    EXPECT_EQ(wire.size(), (layout_.columnCount() + 7) / 8);
    EXPECT_EQ(wire[0], std::byte{0});

    // Deserialize into previous state=true and verify it flips to false.
