...
class TestPandasIntegration(unittest.TestCase):
        ...
        for col in df_original.columns:
            if df_original[col].dtype == object:
                ...
                np.testing.assert_array_equal(
                    df_read[col].values, df_original[col].values
                )
    ...
    def test_large_dataframe(self):
        ...
        pd.testing.assert_frame_equal(
            df_read.head(), df_large.head(), check_dtype=False
        )
        pd.testing.assert_frame_equal(
            df_read.tail(), df_large.tail(), check_dtype=False
        )
    ...
    def test_dataframe_index_handling(self):
        ...
        np.testing.assert_array_equal(
            df_read["name"].values, df_original["name"].values
        )
        np.testing.assert_array_equal(
            df_read["score"].values, df_original["score"].values
        )
    ...
    def test_dataframe_column_order(self):
        ...
        df_original = pd.DataFrame(
            {col: [f"{col}_{i}" for i in range(3)] for col in column_order}
        )
        pybcsv.write_dataframe(df_original, filepath)
        df_read = pybcsv.read_dataframe(filepath)
    ...
    def test_dataframe_special_values(self):
        ...
        self.assertTrue(
            np.isinf(df_read.iloc[1]["value"]) and df_read.iloc[1]["value"] > 0
        )
        self.assertTrue(
            np.isinf(df_read.iloc[2]["value"]) and df_read.iloc[2]["value"] < 0
        )
    ...
    def test_dataframe_performance_comparison(self):
        ...
        manual_data = [
            [row["id"], row["value"], row["category"]] for _, row in df.iterrows()
        ]
