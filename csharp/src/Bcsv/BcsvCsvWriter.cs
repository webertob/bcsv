...
public sealed class BcsvCsvWriter : IDisposable
{
    ...
    public void WriteRow()
    {
        if (!NativeMethods.bcsv_csv_writer_next(_handle))
            NativeMethods.ThrowWithError("CSV WriteRow failed", NativeMethods.bcsv_csv_writer_error_msg(_handle));
    }

    public void Write(BcsvRow row)
    {
        if (!NativeMethods.bcsv_csv_writer_write(_handle, row.Handle))
            NativeMethods.ThrowWithError("CSV Write failed", NativeMethods.bcsv_csv_writer_error_msg(_handle));
    }
